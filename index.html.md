...
<body>  
  ...
  <main class="content">  
    ...
    <section id="content-spese" style="display:none">  
      ...
      <div class="expenses-col">  
        <div class="field-list" id="spese-list">  
          <div class="field-row">  
            <input type="text" placeholder="Etichetta" class="spesa-label" />  
        ...
        <div class="totals-section" id="tot-spese">Totale spese: € 0.00</div>  
  ...
  <script>  
    ...
    let currentSpesaType = 'fisse'; // Fisso di default  
    ...
    function saveCurrentMonthData() {  
      ...
      // Spese fisse (sempre caricate ma salvate tutte)  
      document.querySelectorAll('#spese-list .field-row').forEach(row => {  
        ...
        if(label !== '' && !isNaN(amount)) {  
          if(currentSpesaType === 'fisse') data.speseFisse.push({label, amount});  
          else data.speseVariabili.push({label, amount});  
        }  
      });  
    ...
    function loadMonthData(month) {  
      ...
      // Carica spese fisse e variabili in variabili separate  
      const speseFisse = data?.speseFisse || [];  
      const speseVariabili = data?.speseVariabili || [];  
  
      // Imposta spese correnti in base alla visualizzazione  
      populateSpeseList(currentSpesaType === 'fisse' ? speseFisse : speseVariabili);  
    ...
    function populateSpeseList(spese) {  
      ...
      spese.forEach((entry, i) => {  
        ...
        container.appendChild(newRow);  
        // Set values  
        newRow.querySelector('.spesa-label').value = entry.label || '';  
        newRow.querySelector('.spesa-amount').value = entry.amount || '';  
        addInputListeners(newRow);  
      });  
      if(spese.length === 0) {  
        // Aggiunge due campi vuoti se nessuno spesa ancora  
        for(let i=0; i<2; i++) addSpesaField();  
      }  
    }  
    ...
    function updateAllTotals() {  
      document.getElementById('tot-spese').textContent = "Totale spese: € " + calculateTotaleSpese().toFixed(2);  
    ...
    function updateMonthlyResidual() {  
      ...
      if(prevData) {  
        ...
        const speseSum = (prevData.speseFisse || []).concat(prevData.speseVariabili || []).reduce((acc, s) => acc + s.amount, 0);  
        prevSaldo = entrateSum - speseSum;  
      }  
